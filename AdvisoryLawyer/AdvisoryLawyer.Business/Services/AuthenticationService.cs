...
namespace AdvisoryLawyer.Business.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<AuthenticationModel> LoginWithGmail(string idToken)
        {
            ...
            var account = await _userAccountService.CheckGmail(decodedToken.Claims.GetValueOrDefault("email").ToString(), decodedToken.Claims.GetValueOrDefault("name").ToString(), decodedToken.Claims.GetValueOrDefault("user_id").ToString());
            ...
            if("undefined".Equals(account.Role) || "admin".Equals(account.Role))
            {
                ...
                return new AuthenticationModel {id = account.Id, token = token, display_name = account.Name, email = account.Email, role = account.Role, Uid = account.Uid };
            } 
            else if("customer".Equals(account.Role))
            {
                ...
                return new AuthenticationModel {id = customer.Id, token = token, display_name = customer.Name, email = customer.Email, role = account.Role, Uid = account.Uid };
            }
            else if ("lawyer".Equals(account.Role))
            {
                ...
                return new AuthenticationModel {id = lawyer.Id, token = token, display_name = lawyer.Name, email = lawyer.Email, role = account.Role, Uid = account.Uid };
            }
            else if ("lawyer_office".Equals(account.Role))
            {
                ...
                return new AuthenticationModel {id = lawyerOffice.Id, token = token, display_name = lawyerOffice.Name, email = lawyerOffice.Email, role = account.Role, Uid = account.Uid };
            }
            return null;
        }
    }
